...
class SaveData:
    ...
    def inserir_cliente(self):
        ...
        cliente = self.collection.find_one({"cpf": st.session_state["cpf"]})
        ...
        if not cliente:
                ...
                "conversas": [
                    {
                        ...
                        "chats": [
                            {
                                "data_hora_inicio": st.session_state["data_hora_inicio"],
                                "mensagens": []
                            }
                        ]
                    }
            ...
            assunto_existe = self.collection.find_one({"cpf": st.session_state["cpf"], "conversas.assunto": st.session_state["assunto"]})
            if not assunto_existe:
                ...
                self.collection.update_one(
                    {"cpf": st.session_state["cpf"], "conversas.assunto": st.session_state["assunto"]},
                    {"$push": {
                        "conversas.$.chats": {
                            "data_hora_inicio": st.session_state["data_hora_inicio"],
                            "mensagens": []
    ...
    def inserir_mensagem(self, dict_mensagem):
        ...
        self.collection.update_one(
            ...
            array_filters=[
                {"conversa.assunto": st.session_state["assunto"]},
